...
class RESTMethods {
  ...
  createChannel(guild, name, options) {
    ...
    let tipe = typeof type === 'string' ? type.toUpperCase() : type;
    
    if (typeof tipe !== 'number') {
      tipe = (typeof tipe === 'string' && Object.keys(Constants.ChannelTypes).includes(tipe)) ? Constants.ChannelTypes[tipe] : 0;
    }
    if (![0, 2, 4, 6, 13, 14].includes(tipe)) {
      tipe = 0;
    }
  ...
  updateChannel(channel, _data, reason) {
    ...
    if (typeof _data.type === 'undefined') data.type = channel.type;
    else {
      data.type = typeof _data.type === 'number' ? _data.type : Constants.ChannelTypesArr.indexOf(_data.type);
    }
  ...
  acceptInvite(code) {
    throw new TypeError("This function has been removed.");
  }
