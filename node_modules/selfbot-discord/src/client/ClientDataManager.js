...
class ClientDataManager {
  ...
  async newRelation(data) {
    if (data) {
      let user = this.client.users.has(data.id) ? this.client.users.get(data.id) : this.newUser(data.id);
      let type = Constants.RelationshipTypes[data.type] || data.type;
      let relation = {
        id: data.id,
        type,
        user
      ...
      this.client.emit(Constants.Events.RELATIONSHIP_ADD, relation);
      return relation;
    } else return null;
  }
  ...
  async killRelation(data) {
    ...
    if (!relation.user.id) relation.user = await this.client.fetchUser(data.id);
    this.client.emit(Constants.Events.RELATIONSHIP_REMOVE, relation);
    return relation;
  }
